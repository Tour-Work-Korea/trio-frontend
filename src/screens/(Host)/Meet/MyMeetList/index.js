...
const MyMeetList = () => {
  ...
  const renderItem = ({ item }) => {
    ...
    const timeLabel = `${item.partyStartTime?.slice(0,5)} ~ ${item.partyEndTime?.slice(0,5)}`;
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardInfo}>
          <Text style={[FONTS.fs_12_light, styles.ghName]}>{item.guesthouseName}</Text>
          <Text style={[FONTS.fs_14_medium, styles.title]} numberOfLines={1}>
            {item.partyTitle}
          </Text>
          <Text style={[FONTS.fs_12_medium, styles.time]}>
            {timeLabel}  (총 {Math.max(1, dayjs(`1970-01-01T${item.partyEndTime}`).diff(dayjs(`1970-01-01T${item.partyStartTime}`), 'hour'))}시간)
          </Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <FlatList
          ...
          contentContainerStyle={flatData.length === 0 ? { flex: 1, justifyContent: 'center' } : { paddingBottom: 60 }}
        ...
        >
          <Text style={[FONTS.fs_14_medium, styles.fabText]}>모임 추가하기</Text>
